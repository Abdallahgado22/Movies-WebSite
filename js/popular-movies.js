...
const displayMovies = async (page = 1) => {
  ...
  document.getElementById("popularMovies").innerHTML = container;
  document.getElementById("pageNumber").textContent = `Page ${page}`;
};

// Pagination controls
const setupPagination = (currentPage) => {
  const prevButton = document.getElementById("prevPopularPage");
  const nextButton = document.getElementById("nextPopularPage");
