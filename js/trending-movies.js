...
const apiUrl = `https://api.themoviedb.org/3/trending/movie/week?api_key=${apiKey}&page=`;
...
const displayMovies = async (page = 1) => {
  ...
  document.getElementById("trendingMovies").innerHTML = container;
  document.getElementById("pageNumber").textContent = `Page ${page}`;
};

// Pagination controls
const setupPagination = (currentPage) => {
  const prevButton = document.getElementById("prevTrendingPage");
  const nextButton = document.getElementById("nextTrendingPage");
