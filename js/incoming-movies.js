...
const apiUrl = `https://api.themoviedb.org/3/movie/upcoming?api_key=${apiKey}&language=en-US&page=`;
...
const displayMovies = async (page = 1) => {
  ...
  document.getElementById("incomingMovies").innerHTML = container;
  document.getElementById("pageNumber").textContent = `Page ${page}`;
};

// Pagination controls
const setupPagination = (currentPage) => {
  const prevButton = document.getElementById("prevIncomingPage");
  const nextButton = document.getElementById("nextIncomingPage");
